...
function getAppraisal(){
	...
	if(empselVal!=null && empselVal!=""){
		...
		document.getElementById('desig').innerHTML = "NA";
...
<table style="border: none;" class="col-lg-12" width="100%">
				<tr>
					<td>
					<select class="col-lg-10 form-control" id="empSel" name="empSel"  onChange="clearExistingData()">
							<option value="">Select One Emp</option>
							<c:forEach items="${empList}" var="emp">
								<option value="${emp.coCode}~${emp.joinDate}~${emp.mgrName}~${emp.ecode}~${emp.netId}">${emp.ename}/${emp.hname}</option>
							</c:forEach>
					</select></td>
					<td>
...
<td class="col-sm-1 caption-copy" id="desig">${user.lvlDescMap['Level1Desc']}</td>
</tr>
</table>
</div>
<div class="table-responsive">
